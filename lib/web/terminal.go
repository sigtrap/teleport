...
func (t *TerminalHandler) streamTerminal(ws *websocket.Conn, tc *client.TeleportClient) {
	...
	//
	if err != nil && strings.Contains(err.Error(), teleport.ServerIsAmbiguous) {
		t.log.Infof("Ambiguous hostname %q, attempting to connect by UUID (%q)", t.hostName, t.hostUUID)
		tc.Host = t.hostUUID
